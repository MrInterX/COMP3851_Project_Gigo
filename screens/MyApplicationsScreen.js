...
export default function MyApplicationsScreen({ navigation }) {
  ...
  // 每次返回这个页面时刷新一次（比如从 ApplicationDetail 删掉后回来）
  useFocusEffect(
    useCallback(() => {
      loadApplications();
    }, [loadApplications])
  ...
  // 跳到 “申请详情” 页面，而不是原来的 JobDetail
  const handleViewJob = (jobId, applicationId) => {
    if (!jobId || !applicationId) return;
    navigation.navigate("ApplicationDetail", { jobId, applicationId });
  };

  // 删除申请（按 application.id 删除）
      ...
      <View style={styles.cardBottomRow}>
        ...
        <View style={styles.cardBtnRow}>
          <TouchableOpacity
            ...
            onPress={() => onView(item.job_id, item.id)} // ✅ 传 jobId + applicationId
          >
          ...
          <TouchableOpacity
            ...
            onPress={() => onDelete(item.id)} // ✅ 删除传 application.id
          >
            <Ionicons
              name="trash-outline"
              size={16}
